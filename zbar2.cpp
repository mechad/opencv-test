...
std::vector<std::vector<cv::Point>>  detectContours(cv::Mat gray) {
    cv::Mat binary;
    cv::adaptiveThreshold(gray, binary, 255, cv::ADAPTIVE_THRESH_MEAN_C, cv::THRESH_BINARY, 11, 2);
    // 膨胀处理
    cv::Mat dilated;
    cv::dilate(binary, dilated, cv::Mat(), cv::Point(-1, -1), 2);

    std::vector<std::vector<cv::Point>> contours;
    cv::findContours(dilated, contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);

    return contours;
    // cv::Mat contourImage = cv::Mat::zeros(gray.size(), CV_8UC3);
    // cv::drawContours(gray, contours, -1, cv::Scalar(0, 255, 0), 2);
...
void detectAndCorrectBarcode(cv::Mat &image) {

    cv::Mat edges;
    cv::Canny(image, edges, 50, 150);

    std::vector<cv::Vec4i> lines;
    cv::HoughLinesP(edges, lines, 1, CV_PI / 180, 100, 10, 10);
    ...
    for (size_t i = 0; i < lines.size(); i++) {
        ...
        if (std::abs(theta) > 45 && std::abs(theta) < 135) {
            angle += theta;
            count++;
        }
    }
    ...
    if (count > 0) {
        angle /= count;
        cv::Mat rotationMatrix = cv::getRotationMatrix2D(cv::Point(image.cols / 2, image.rows / 2), angle, 1);
        cv::Mat rotatedImage;
...
int main(int argc, char** argv) {
    ...
    while (cap.read(frame)) {
        ...
        if (maxVal > 0.4)
        {
            ...
            barcodeRect.x = 700;
            barcodeRect.y = 300;
            barcodeRect.width = 40;
            barcodeRect.height = 350;
            ...
            // mythreshold(rotatedImg, 180); // 识别率99.52%, 耗时：0.0026s
            cv::threshold(rotatedImg, rotatedImg, 180, 255, cv::THRESH_BINARY); // 识别率98.56%, 耗时：0.0024s
            // cv::adaptiveThreshold(rotatedImg, rotatedImg, 255, cv::ADAPTIVE_THRESH_MEAN_C, cv::THRESH_BINARY, 11, 2); // 识别率0 %, 耗时：0.0032s
            // cv::threshold(rotatedImg, rotatedImg, 140, 255, cv::THRESH_BINARY);
            // std::vector<std::vector<cv::Point>> contours = detectContours(rotatedImg);
            // cv::drawContours(rotatedImg, contours, -1, cv::Scalar(0, 255, 0), 2);
            detectAndCorrectBarcode(rotatedImg);
